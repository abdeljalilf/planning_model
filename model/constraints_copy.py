...
def add_constraints(model, data):
    ...
    def demande_avec_pertes_rule(m, k):
        return sum(eta_ih[h, i] * m.x_ihkt[i, h, k, t] for i in I for h in H for t in T if U_ih[h, i] == 1) == D_k[k]
    ...
    def mu_ck_rule(m, c, k):
        return m.mu_ck[c, k] == sum(
            ...
            for i in I for h in H for t in T if U_ih[h, i] == 1
        )
    ...
    # model.LiaisonY = Constraint(model.I, model.H, model.K, model.T, rule=y_upper_rule)

    # def y_def_rule(m, i, h, k, t):
    #     if U_ih[h, i] == 1:
    #         return m.y_ihkt[i, h, k, t] == m.x_ihkt[i, h, k, t] / M_max
    #     return Constraint.Skip
    # model.DefinitionY = Constraint(model.I, model.H, model.K, model.T, rule=y_def_rule)
    ...
    #     return Constraint.Skip
    # model.LiaisonZ = Constraint(model.I, model.H, model.K, model.T, rule=z_lower_rule)

    # def z_def_rule(m, i, h, k, t):
    #     if U_ih[h, i] == 1:
    #         return m.z_ihkt[i, h, k, t] == m.x_ihkt[i, h, k, t] / M_min
    #     return Constraint.Skip
    # model.DefinitionZ = Constraint(model.I, model.H, model.K, model.T, rule=z_def_rule)
    ...
    model.FenetreLivraison_inf = Constraint(model.I, model.H, model.K, model.T, rule=fenetre_livraison_inf_rule)
    model.FenetreLivraison_sup = Constraint(model.I, model.H, model.K, model.T, rule=fenetre_livraison_sup_rule)
